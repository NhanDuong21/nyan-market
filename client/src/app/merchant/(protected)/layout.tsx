...
export default function MerchantLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
      {/* GLOBAL LOADING OVERLAY: Prevents UI interaction and layout flashing */}
      {!isAuthorized && (
        <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-gray-50">
          <div className="h-16 w-16 animate-spin items-center justify-center rounded-full border-4 border-primary-100 border-t-primary-500"></div>
      ...
      {/* CSS hiding mechanism prevents Next.js layout teardown on navigation */}
      <div className={`flex min-h-screen bg-gray-50/50 transition-opacity duration-300 ${!isAuthorized ? "invisible opacity-0" : "visible opacity-100"}`}>
        {/* Sidebar */}
